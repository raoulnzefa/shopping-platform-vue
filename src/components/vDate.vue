...
<template>
  <div class="cov-vue-date" v-show="!!option">
      ...
      <div class="cov-date-body" :style="{'background-color': option.color ? option.color.header : '#3f51b5'}">
        ...
        <div class="cov-date-box" v-if="showInfo.day">
          <div class="cov-picker-box">
            ...
            <div class="day" v-for="(day,index) in dayList" :key="index" track-by="$index" @click="checkDay(day)" :class="{'checked':day.checked,'unavailable':day.unavailable,'passive-day': !(day.inMonth)}" :style="day.checked ? (option.color && option.color.checkedDay ? { background: option.color.checkedDay } : { background: '#F50057' }) : {}">{{day.value}}</div>
          </div>
        </div>
        <div class="cov-date-box list-box" v-if="showInfo.year">
          <div class="cov-picker-box date-list" id="yearList">
            <div class="date-item" v-for="(yearItem,index) in library.year" :key="index" track-by="$index" @click="setYear(yearItem)">{{yearItem}}</div>
          </div>
        </div>
        <div class="cov-date-box list-box" v-if="showInfo.month">
          <div class="cov-picker-box date-list">
            <div class="date-item" v-for="(monthItem,index) in library.month" :key="index" track-by="$index" @click="setMonth(monthItem)">{{monthItem}}</div>
          </div>
        </div>
  ...
  methods: {
    ...
    showDay (time) {
      ...
      for (let i = 1; i <= monthDays; ++i) {
        ...
        if (
          i === Math.ceil(moment(currentMoment).format('D')) &&
          moment(oldtime, this.option.format).year() ===
            moment(currentMoment).year() &&
          moment(oldtime, this.option.format).month() ===
            moment(currentMoment).month()
        ) {
    ...
    checkBySelectDays (d, days) {
      this.selectedDays.forEach(day => {
        if (
          this.checked.year === moment(day).format('YYYY') &&
          this.checked.month === moment(day).format('MM') &&
    ...
    limitWeekDay (limit, days) {
      days.map(day => {
        if (
          limit.available.indexOf(Math.floor(day.moment.format('d'))) === -1
        ) {
    ...
    setYear (year) {
      this.checked.currentMoment = moment(
        year + '-' + this.checked.month + '-' + this.checked.day
      )
    ...
    setMonth (month) {
      ...
      this.checked.currentMoment = moment(
        this.checked.year + '-' + mo + '-' + this.checked.day
      )
      this.showDay(this.checked.currentMoment)
    },
    showCheck () {
    ...
    picked () {
      if (this.option.type === 'day' || this.option.type === 'min') {
        ...
        this.date.time = moment(this.checked.currentMoment).format(
          this.option.format
        )
      } else {
    ...
    shiftActTime () {
      ...
      this.$nextTick(() => {
        ...
        document.querySelector('.hour-box').scrollTop =
          (document.querySelector('.hour-item.active').offsetTop || 0) - 250
        document.querySelector('.min-box').scrollTop =
          (document.querySelector('.min-item.active').offsetTop || 0) - 250
      })
    }
  }
}
